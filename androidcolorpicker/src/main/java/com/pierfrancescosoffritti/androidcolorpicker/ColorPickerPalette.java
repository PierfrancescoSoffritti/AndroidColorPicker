...
public class ColorPickerPalette extends TableLayout {
     ...
     */
    public void init(@ColorPickerDialog.SwatchesSize int size, @IntRange(from = 1) int columns, ColorPickerSwatch.OnColorSelectedListener listener) {
        if(columns <= 0)
            throw new IllegalArgumentException("column number <= 0");
     ...
     * Adds swatches to table in a serpentine format.
     */
    public void drawPalette(int[] colors, int selectedColor, @Nullable String[] colorContentDescriptions) {
        if (colors == null) {
            return;
        }
        ...
        for (int color : colors) {
            ...
            if(colorContentDescriptions != null)
